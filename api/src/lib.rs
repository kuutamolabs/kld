                ...
                NetAddressHelper::Hostname((port, hostname)) => {
                    let hostname = lightning::util::ser::Hostname::try_from(hostname.clone())
                        .unwrap_or_else(|_| unsafe {
                            // The invalid hostname may come from interenet, and leave it as it is,
                            // no panic, such that the program will not break with a invalid internet
                            // packet
                            eprintln!("invalid hostname detected: {:?}", hostname);
                            std::mem::transmute::<(String,), lightning::util::ser::Hostname>((
                                hostname,
                            ))
                        });
                    lightning::ln::msgs::NetAddress::Hostname { hostname, port }
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        if let Some((ip, port)) = s.rsplit_once(':') {
            if let Ok(ipv4) = Ipv4Addr::from_str(ip) {
                return Ok(NetAddress(lightning::ln::msgs::NetAddress::IPv4 {
                    addr: ipv4.octets(),
                    port: port.parse()?,
                }));
            } else if let Ok(ipv6) = Ipv6Addr::from_str(ip) {
                return Ok(NetAddress(lightning::ln::msgs::NetAddress::IPv6 {
                    addr: ipv6.octets(),
                    port: port.parse()?,
                }));
            }
        }
        anyhow::bail!("Invalid network address:port")
    }
}
