...
impl LSPSMessage {
    pub fn from_str_with_id_map(
        json_str: &str,
        request_id_to_method: &mut HashMap<String, String>,
    ) -> Result<Self, serde_json::Error> {
        let deserializer = &mut serde_json::Deserializer::from_str(json_str);
        let visitor = LSPSMessageVisitor {
            request_id_to_method,
        };
...
impl<'de, 'a> Visitor<'de> for LSPSMessageVisitor<'a> {
        ...
        A: MapAccess<'de>,
    {
        ...
        match (id, method) {
            ...
            (None, Some(method)) => Err(de::Error::custom(format!(
...
mod tests {
    ...
    fn deserializes_success_response() {
        ...
        request_id_to_method_map.insert(
            "request:id:xyz123".to_string(),
            "lsps0.listprotocols".to_string(),
        );
