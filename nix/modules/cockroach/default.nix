...
let
  cockroach = pkgs.callPackage ../../pkgs/cockroach.nix { };
...

{
  options = {
    kuutamo.cockroach = {
      ...
      package = mkOption {
        type = types.package;
        default = config.kuutamo.pkgs.cockroach;
        defaultText = literalExpression "config.kuutamo.pkgs.cockroach";
        description = ''
  ...
  config = mkIf config.kuutamo.cockroach.enable {
    assertions = [
      {
