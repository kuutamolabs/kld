{ lib, config, ... }: {
  ...
  options.kuutamo.console.set = lib.mkOption {
    type = lib.types.bool;
    default = false;
    description = lib.mdDoc ''set default consoles'';
  };
  ...
  config = {
    ...
    srvos.boot.consoles = {
      _type = "override";
      content = config.kuutamo.console.defaults;
      priority =
        if config.kuutamo.console.set
        then
          999
        else
        # will not over write any other Default values
          1001;
    };
