...
let
  ...
  makeNode = locality: myAddr: joinNode:
    { self, nodes, pkgs, lib, config, ... }:
    {
      ...
      # In order to ensure that Chrony has performed its
      ...
      kuutamo.cockroachdb.insecure = true;
      kuutamo.cockroachdb.openPorts = true;
      kuutamo.cockroachdb.locality = locality;
      kuutamo.cockroachdb.listen.address = myAddr;
      kuutamo.cockroachdb.join = lib.mkIf (joinNode != null) joinNode;
